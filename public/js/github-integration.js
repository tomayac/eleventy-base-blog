	...
	try {
		...
		for (const img of (draft.imageFiles || [])) {
			...
			if (data) {
				...
				await ghFetch(ui, `/contents/${imgPath}`, { method: 'PUT', body: JSON.stringify({ message: `${img.sha ? 'Update' : 'Add'} image ${img.name}`, content, branch: branchName, sha: img.sha }) });
			}
		}

		await ghFetch(ui, `/contents/${draft.path || `content/blog/${slug}/${slug}.md`}`, { method: 'PUT', body: JSON.stringify({ message: `${draft.sha ? 'Update' : 'Add'} post ${slug}`, content: toBase64(md), branch: branchName, sha: draft.sha }) });

		const pr = await ghFetch(ui, '/pulls', { method: 'POST', body: JSON.stringify({ title: `${draft.sha ? 'Update' : 'Post'}: ${ui.titleInput.value}`, head: branchName, base: defaultBranch }) });
...
export async function loadPostFromGitHub(ui, path) {
	const { ghOwnerInput: owner, ghRepoInput: repo, ghTokenInput: token } = ui;
	if (!owner.value || !repo.value || !token.value) throw new Error('Please fill in GitHub settings first.');
	...
	try {
		...
		frontMatter.split('\n').forEach(line => {
			...
			if (value.startsWith('[') && value.endsWith(']')) value = value.substring(1, value.length - 1).split(',').map(t => t.trim().replace(/^["']|["']$/g, ''));
			data[key] = value;
		});
		...
		try {
			...
			const imageFiles = dirContents.filter(f => f.type === 'file' && /\.(jpe?g|png|gif|webp|svg)$/i.test(f.name));
