...
import { saveBackendConfigs, updateBackendFields, updateGlobalConfig } from './ai-config.js';
...
export function initSettingsFileHandler(ui) {
	...
	ui.saveSettingsBtn.onclick = async () => {
		const settings = {
			...
			'ai-features-enabled': localStorage.getItem('ai-features-enabled') === 'true',
			'ai-only-existing-tags': localStorage.getItem('ai-only-existing-tags') === 'true',
			'ai-backend': localStorage.getItem('ai-backend'),
			'ai-backend-configs': JSON.parse(localStorage.getItem('ai-backend-configs') || '{}')
		};
	...
	ui.loadSettingsBtn.onclick = async () => {
		try {
			...
			if (settings['ai-features-enabled'] !== undefined) {
				localStorage.setItem('ai-features-enabled', settings['ai-features-enabled']);
				ui.aiFeaturesToggle.checked = settings['ai-features-enabled'];
			}
			
			if (settings['ai-only-existing-tags'] !== undefined) {
				localStorage.setItem('ai-only-existing-tags', settings['ai-only-existing-tags']);
