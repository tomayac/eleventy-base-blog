	...
	ui.aiSuggestTagsBtn.onclick = async () => {
		...
		try {
			...
			const runRestricted = async () => {
				...
				const session = await LanguageModel.create(opts); let full = '';
				const stream = session.promptStreaming(`Content: ${content}`, { responseConstraint: schema });
				for await (const chunk of stream) { full += chunk; try { addTags(JSON.parse(full).tags); } catch (e) {} }
			};
			const runFreeform = async () => {
				const opts = { initialPrompts: [{ role: 'system', content: `Suggest 3-5 tags for this blog post in ${lang}. Return JSON: {"tags": ["tag1", "tag2"]}.` }] };
				const session = await LanguageModel.create(opts); let full = '';
